...
module Violation : sig
  type violation =
    | Not_a_subjkind of l * r
    | No_intersection of r * r
...
val of_type_decl :
  ...
  (l * annotation * Parsetree.attributes) option
...
(** Gets the maximum mode on the externality axis for types of this jkind.
    Works only on [r] jkinds because the upper bound might depend on types
    for [l] kinds. *)
...
val set_externality_upper_bound : r -> Externality.t -> r

(** Does this jkind classify a type known to be external? [false] is a safe
    answer if we're not sure. *)
val is_definitely_external : 'd t -> bool
...
(** Checks whether two jkinds have a non-empty intersection. Might mutate
    sort variables. *)
val has_intersection : r -> r -> bool

(** Like [has_intersection], but comparing two [l] jkinds. *)
val has_intersection_l_l : l -> l -> bool
...
val intersection_or_error :
  reason:History.interact_reason -> r -> r -> (r, Violation.t) Result.t
...
val sub_jkind_l : l -> l -> (l, Violation.t) result
...
val intersect_l_l :
  reason:History.interact_reason -> l -> l -> (l, Violation.t) result
