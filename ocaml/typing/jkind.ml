...
module Const = struct
  ...
  let jkind_of_product_annotations _jkinds = assert false
  (* CR reisenberg: implement me
  ...
  let rec of_user_written_annotation_unchecked_level ~transl_type
    ...
    | Product ts ->
      let jkinds =
        List.map (of_user_written_annotation_unchecked_level ~transl_type) ts
      in
  ...
  *)
  ...
  let of_user_written_annotation ~transl_type ~context
...
module Desc = struct
  ...
  let rec sub ~type_equal d1 d2 : Misc.Le_result.t =
    match d1, d2 with
    ...
    | Product ds1, Product ds2 ->
      ...
      then Misc.Le_result.combine_list (List.map2 (sub ~type_equal) ds1 ds2)
      else Not_le
    ...
    let folder _ =
      ...
      (* CR reisenberg: implement
         ...
         ( layout :: layouts,
           ...
           Nullability.join null_ub nullability_upper_bound ) *)
    in
    let layouts, upper_bounds = List.fold_left folder ([], Bounds.min) jkinds in
    { layout = Product (List.rev layouts); upper_bounds }

  (* Post-condition: If the result contains [Var v], then [!v] is [None]. *)
  let rec get_sort upper_bounds s : Desc.t =
    match Sort.get s with
    | Base b -> Const { layout = Base b; upper_bounds }
    | Var v -> Var v
    | Product sorts ->
      Desc.Product (List.map (fun x -> get_sort upper_bounds x) sorts)

  let rec get ({ layout; upper_bounds } as k) : Desc.t =
    match layout with
    | Any -> Const { layout = Any; upper_bounds }
    | Sort s -> get_sort upper_bounds s
    | Product layouts ->
...
let of_type_decl ~transl_type ~context (decl : Parsetree.type_declaration) =
  let jkind_of_annotation =
    ...
    |> Option.map (fun (annot, attrs) ->
           let t, const =
             of_annotation ~transl_type:(Some transl_type) ~context annot
           in
           t, const, attrs)
  in
...
let of_type_decl_default ~transl_type ~context ~default
...
let default_to_value_and_get { jkind = { layout; upper_bounds }; _ } : Const.t =
  { layout = Layout.default_to_value_and_get layout; upper_bounds }
...
let equal ~type_equal t1 t2 =
  equate_or_equal ~allow_mutation:true ~type_equal t1 t2

let () = Types.set_jkind_equal (fun ~type_equal -> equal ~type_equal)

let equate ~type_equal t1 t2 =
  equate_or_equal ~allow_mutation:true ~type_equal t1 t2
...
let combine_histories ~type_equal reason (Pack k1) (Pack k2) =
  if flattened_histories
  then
    match
      Desc.sub ~type_equal (Jkind_desc.get k1.jkind) (Jkind_desc.get k2.jkind)
    with
    | Less -> k1.history
    | Not_le ->
      k2.history
...
let sub_or_intersect ~type_equal t1 t2 =
...
let sub_or_error ~type_equal t1 t2 =
  match sub_or_intersect ~type_equal t1 t2 with
  | Sub -> Ok ()
...
let sub_jkind_l ~type_equal sub super =
  ...
  | Less | Equal ->
    Ok
      { sub with
        history = combine_histories ~type_equal Subjkind (Pack sub) (Pack super)
      }
  ...
  let rec history print_type_expr ppf = function
    | Interact
      ...
      fprintf ppf
        ...
        jkind1 (history print_type_expr) history1
        (Jkind_desc.Debug_printers.t print_type_expr)
        jkind2 (history print_type_expr) history2
    | Creation c -> fprintf ppf "Creation (%a)" creation_reason c

  let t print_type_expr ppf ({ jkind; history = h; has_warned = _ } : 'd t) :
      unit =
