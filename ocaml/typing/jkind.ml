...
module Jkind_desc = struct
  ...
  let rec get
      ({ layout; upper_bounds } as k) : Desc.t =
    match layout with
    | Any ->
      Const
        { layout = Any; upper_bounds }
    | Sort s ->
      get_sort modes_upper_bounds externality_upper_bound
...
let combine_histories ~type_equal reason (Pack k1) (Pack k2) =
  if flattened_histories
  then
    match Desc.sub ~type_equal (Jkind_desc.get k1.jkind) (Jkind_desc.get k2.jkind) with
    | Less -> k1.history
    | Not_le ->
      k2.history
...
let sub_jkind_l ~type_equal sub super =
  ...
  | Less | Equal ->
    Ok { sub with history = combine_histories ~type_equal Subjkind (Pack sub) (Pack super) }
...
module Debug_printers = struct
  ...
  let rec history print_type_expr ppf = function
    | Interact
      ...
      fprintf ppf
        "Interact {@[reason = %a;@ jkind1 = %a;@ history1 = %a;@ \
         jkind2 = %a;@ history2 = %a}@]"
  ...
  let t print_type_expr ppf ({ jkind; history = h; has_warned = _ } : 'd t) : unit
      =
