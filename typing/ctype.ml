...
let rec extract_concrete_typedecl env ty =
  ...
  | Tvariant _ | Tpackage _ | Tmod_modes _ -> Has_no_typedecl
...
let contained_without_boxing env ty =
  ...
  | Tsubst _ | Tvariant _ | Tunivar _ | Tpackage _ | Tmod_modes _ -> []
...
let rec estimate_type_jkind ~expand_component env ty =
  ...
  | Tmod_modes (layout, modes) -> Jkind.for_type_mod_modes layout modes
...
let rec eqtype rename type_pairs subst env ~do_jkind_check t1 t2 =
  ...
  *)
  ...
  try
    ...
    | (Tconstr (p1, [], _), Tconstr (p2, [], _)) when Path.same p1 p2 ->
        ()
    | (Tmod_modes (l1, m1), Tmod_modes (l2, m2)) ->
      if not (Jkind.Mod_bounds.equal m1 m2) || not (Jkind.Layout.equate l1 l2)
...
let rec build_subtype env (visited : transient_expr list)
  ...
  | Tunivar _ | Tpackage _ | Tmod_modes _ -> (t, Unchanged)
