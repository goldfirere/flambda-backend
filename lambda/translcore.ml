...
and transl_exp0 ~in_new_scope ~scopes sort e =
  ...
  | Texp_construct(_, cstr, args, alloc_mode) ->
      let args_with_sorts =
        List.map2 (fun { ca_jkind } e ->
            ...
            let sort = Jkind.Sort.assert_const sort in
            e, sort)
          cstr.cstr_args args
      in
      let ll =
  ...
  | Texp_field(arg, id, lbl, float, ubr) ->
      ...
      let lbl_sort = Jkind.Sort.assert_const lbl_sort in
  ...
  | Texp_letop{let_; ands; param; param_sort; body; body_sort; partial} ->
      let param_sort = Jkind.Sort.default_for_transl_and_get param_sort in
...
and transl_letop ~scopes loc env let_ ands param param_sort case case_sort
  ...
  let rec loop prev_layout prev_lam = function
    ...
    | and_ :: rest ->
        ...
        in
        let bop_op_return_sort =
          Jkind.Sort.default_for_transl_and_get and_.bop_op_return_sort
        in
        let exp = transl_exp ~scopes bop_exp_sort and_.bop_exp in
        let right_layout = layout_exp bop_exp_sort and_.bop_exp in
        let result_layout =
          function2_return_layout env and_.bop_loc bop_op_return_sort
            and_.bop_op_type
        in
        let lam =
